...
/// Delete a post
pub fn delete_post(id: i32, conn: &PooledPg) -> Result<Post> {
        debug!("Delete blog post {}", &id);
...
mod tests {
        ...
        #[test]
        fn it_deletes_blog_post(new in arb_new_post()) {
                let conn = test_connection();

                let inserted_post = insert_post(new, &conn).expect("update_post should not fail");

                let deleted_post = delete_post(inserted_post.id, &conn).expect("delete_post should not fail");

                assert_eq!(inserted_post, deleted_post)
        }
        }
}
